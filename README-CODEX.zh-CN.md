...
基线为原版 v1.0.0，提交 `984023ddac0d5e27624f2baacde6105e45de375f`。原版 417 个受版本控制的文件逐字节保留，包括原 LICENSE。公开版是可追溯到该上游提交的源码快照；不声称保留上游 Git 历史。新增内容都在适配层中。可以继续在同一项目使用原版 Claude Code 入口。

**当前结论：原始内容完整保留，适配层已实现并完成自动检查与有限行为抽测；尚不能称为所有引擎、所有工作流都已验证的 100% 运行等价版本。** 详细证据见 [验证报告](docs/codex-adapter/validation.md)。
...
1. 在 Codex 中将 **这个 `ccgs-codex` 文件夹本身**打开为项目，并在该项目中新建任务。当前外层调研任务不会自动取得另一个目录的技能和钩子。
...
- **模型：**默认继承你选择的 Codex 模型。Claude 的 opus / sonnet / haiku 元数据保留，不直接当成 Codex 模型名。本机旧 CLI 不能调用当前桌面使用的 `gpt-6-astra`；只读抽测临时指定 `gpt-5.5` 成功，未改你的默认配置。应使用支持所选模型的 CLI/桌面版本。
...
`doctor` 会区分结构通过和运行未验证。严格检查用于验证交付版没有改动原始 417 个文件；开始游戏后按原流程填写文档、状态或偏好，会出现预期的原始文件差异，应审阅这些差异，不能把它们都当成故障。
