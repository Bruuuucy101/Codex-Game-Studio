# libgdx-scene2d-specialist — behavioral specification

Source: `.claude/agents/libgdx-scene2d-specialist.md`. Run with the full canonical body and runtime
...
**Input:** Review a custom Actor.draw that calls batch.begin and a hidden screen still consuming input.

**Expected:** Allows the override but catches the already-active Batch contract; restores draw state and requires processor cleanup, focus/layout tests on a real backend.
