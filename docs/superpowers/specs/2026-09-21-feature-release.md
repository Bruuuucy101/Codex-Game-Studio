...
Update English and Chinese entry guides with concrete commands, actual prerequisites, preview/write behavior and tested limits. Update the existing 34-issue audit dispositions and aggregate counts rather than leaving delivered features marked deferred. Historical v0.1.1 evidence remains explicitly historical. New validation records actual test/build/behavior evidence; links resolve inside the published repository or to authoritative public sources.

Sample canonical workflows with fresh agents in isolated fixture projects: web version/ownership consultation, engine-agnostic tool setup and mixed-game preservation, and libGDX headless-versus-rendering reasoning. Record exact source hashes, accepted fixture instructions and observed outcomes. These samples do not certify every role/workflow. No self-review is called an independent role review.

Before release, perform a whole-branch review of the aggregate change after task-scoped reviews. Resolve findings with one combined fix wave and scoped rereview per SDD. Re-run affected checks only when fixes require them. Final source tree must match candidate/public tree by bytes and intended modes; preserve existing remote executable modes where the prior publication differs from local history. Never force overwrite concurrent public changes.

After main integration, verify exact main CI and release target commit, download source ZIP/checksum, compare archive SHA256 and every file's Git blob SHA/mode/inventory against the expected public tree, and reject extras/missing files. Archive local evidence/receipts outside the product source. Completion requires published, tested, inspectable results with known limitations clearly stated.
