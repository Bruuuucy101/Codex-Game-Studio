...
# CCGS Codex entry: bug-triage

Source: `.claude/skills/bug-triage/SKILL.md` (relative to the project root).
SHA256: `cdafaffc22abe4f6ee9dc7d93b34222547dd3281c25e60de80d1876517a395d2`
Original metadata: {"name": "bug-triage", "description": "Read all open bugs in production/qa/bugs/, re-evaluate priority vs. severity, assign to sprints, surface systemic trends, and produce a triage report. Run at sprint start or when the bug count grows enough to need re-prioritization.", "argument-hint": "[sprint | full | trend]", "user-invocable": "true", "allowed-tools": "Read, Glob, Grep, Write, Edit", "model": "sonnet"}
...
3. Read the COMPLETE `.claude/skills/bug-triage/SKILL.md` workflow, in chunks if needed; do not execute from this entry or description alone. Read its required references and matched rules. This entry is not a condensed replacement.
