...
# CCGS Codex entry: scope-check

Source: `.claude/skills/scope-check/SKILL.md` (relative to the project root).
SHA256: `f91150df11aadf32cfa64a1476c608396e12eeeba9aca03836c52299f3a0efa0`
Original metadata: {"name": "scope-check", "description": "Analyze a feature or sprint for scope creep by comparing current scope against the original plan. Flags additions, quantifies bloat, and recommends cuts. Use when user says 'any scope creep', 'scope review', 'are we staying in scope'.", "argument-hint": "[feature-name or sprint-N]", "user-invocable": "true", "allowed-tools": "Read, Glob, Grep, Bash", "model": "haiku"}
...
3. Read the COMPLETE `.claude/skills/scope-check/SKILL.md` workflow, in chunks if needed; do not execute from this entry or description alone. Read its required references and matched rules. This entry is not a condensed replacement.
